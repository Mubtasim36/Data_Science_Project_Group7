...
packages <- c("ggplot2", "dplyr","reshape2","outliers","e1071","corrplot","caret", "rpart")
...
trainIndex <- createDataPartition(New_employees$Department, p = 0.7, list = FALSE)  #Splits the data stratified by class (Department).   p = 0.7 → 70% training, 30% testing.   Maintains class balance in both sets.
trainData <- New_employees[trainIndex, ]
testData  <- New_employees[-trainIndex, ]   #Creates training and testing datasets.
...
library(rpart)
model_dt <- rpart(
  Department ~ Age + Experience_Years + Salary + Gender+Performance_Score,  #using all except ID because: ID are all unique, so running that for tree will use much cpu and make Rstudio freeze
  data = trainData,
  method = "class",
